...
import type {
  ...
  Vote,
  UserExpertise
} from '../schemas/CommunityVotingSchemas';
...
export interface VoteWeightBreakdown {
  userId: string;
  baseWeight: number;          // 0.5-1.0 based on account age
  expertiseWeight: number;     // 0-0.5 based on category expertise
  consistencyBonus: number;    // 0-0.25 based on consensus alignment
  totalWeight: number;         // Final weight (0.5-2.0)
...
export interface WeightedVoteResult {
  ...
  confidence: number;          // 0-100, how confident in decision
...
export class BayesianVotingEngine {
  ...
  private readonly MIN_WEIGHT = 0.5;      // Minimum vote weight
  private readonly MAX_WEIGHT = 2.0;      // Maximum vote weight
  private readonly MIN_VOTES_FOR_DECISION = 5;  // Need at least 5 votes

  // Decision thresholds
  private readonly ACCEPT_THRESHOLD = 0.75;     // 75% weighted helpful → accept
  private readonly REJECT_THRESHOLD = 0.40;     // <40% weighted helpful → reject
  private readonly REVIEW_THRESHOLD = 0.60;     // 40-75% → needs review
   ...
   */
  private calculateBaseWeight(accountAgeDays: number): number {
   ...
   */
  private calculateConsistencyBonus(consensusRate: number): number {
  ...
  applyWeightedVoting(
    ...
    votes: Vote[],
    userExpertiseMap: Map<string, UserExpertise>
  ): WeightedVoteResult {
    const reasoning: string[] = [];
    ...
    // Make decision
    let action: WeightedVoteResult['action'] = 'pending';
    let confidence = 0;
  ...
  updateUserExpertise(
    ...
    const expertiseValues = Object.values(updated.categoryExpertise) as number[];
