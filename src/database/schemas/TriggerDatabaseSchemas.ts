/**
 * TRIGGER DATABASE SCHEMAS
 *
 * SQL schemas for the database-driven warning system.
 * Two main tables:
 * 1. media_content - Stores media entries with platform IDs
 * 2. overall_triggers - Stores severity levels (0-3) per category
 * 3. timestamp_triggers - Stores precise timestamp ranges for triggers
 *
 * Created for: Trigger Warnings Extension
 * Date: 2025-12-08
 */
...
/**
 * Generate severity column names for all categories
 */
export function generateSeverityColumns(): string {
  return CATEGORY_KEYS.map(
    (category) =>
      `  ${category}_severity INTEGER DEFAULT 0 CHECK (${category}_severity >= 0 AND ${category}_severity <= 3)`
  ).join(',\n');
}
...
export const SQL_SCHEMAS = {
  /**
   * Media Content Table
   * Central table for all media entries
   */
  media_content: `
-- Media Content Table
-- Stores all media entries with platform-specific IDs for cross-platform reconciliation
...
CREATE TABLE IF NOT EXISTS media_content (
  ...
  -- Platform-specific IDs (NULL if not available on that platform)
...
CREATE INDEX IF NOT EXISTS idx_media_series ON media_content(series_imdb_id) WHERE series_imdb_id IS NOT NULL;
...
CREATE TRIGGER trigger_media_content_updated_at
  ...
  /**
   * Overall Triggers Table
   * Stores severity levels (0-3) for each trigger category per media
   */
  overall_triggers: `
-- Overall Triggers Table
-- Stores trigger severity levels (0-3) for each category
-- 0 = No trigger, 1 = Mild, 2 = Moderate, 3 = Severe
...
-- Index for finding media with specific triggers
...
CREATE TRIGGER trigger_overall_triggers_updated_at
  ...
  /**
   * Timestamp Triggers Table
   * Stores precise timestamp ranges for each trigger occurrence
   */
  timestamp_triggers: `
-- Timestamp Triggers Table
-- Stores precise start/end times for each trigger occurrence in a media
...
CREATE TABLE IF NOT EXISTS timestamp_triggers (
  ...
  CONSTRAINT valid_category CHECK (category IN (
    'violence', 'blood', 'gore', 'sexual_assault', 'sex', 'self_harm', 'suicide',
    'eating_disorders', 'drugs', 'swear_words', 'spiders_snakes', 'animal_cruelty',
    'child_abuse', 'children_screaming', 'domestic_violence', 'racial_violence',
    'lgbtq_phobia', 'religious_trauma', 'dead_body_body_horror', 'torture', 'murder',
    'detonations_bombs', 'medical_procedures', 'vomit', 'flashing_lights', 'jumpscares',
    'natural_disasters', 'cannibalism', 'gunshots', 'explosions', 'screams', 'slurs',
    'hate_speech', 'threats', 'photosensitivity', 'loud_noises', 'insects_spiders',
    'snakes_reptiles', 'needles_injections', 'pregnancy_childbirth', 'death_dying',
    'claustrophobia_triggers', 'physical_violence', 'car_crashes'
  ))
);
...
CREATE TRIGGER trigger_timestamp_triggers_updated_at
  ...
  /**
   * Timestamp Votes Table
   * Tracks user votes on timestamp triggers
   */
  timestamp_votes: `
-- Timestamp Votes Table
-- Tracks community votes on timestamp triggers for accuracy
...
CREATE TABLE IF NOT EXISTS timestamp_votes (
  ...
  -- One vote per user per timestamp
  UNIQUE(timestamp_id, user_id)
);

-- Index for finding votes
...
-- Function to update vote counts on timestamp_triggers
...
-- Authenticated users can insert timestamp triggers
...
-- Moderator policies (assumes 'moderator' role)
CREATE POLICY "Moderators can update media_content"
  ON media_content FOR UPDATE
  USING (auth.jwt() ->> 'role' = 'moderator');

CREATE POLICY "Moderators can update overall_triggers"
  ON overall_triggers FOR ALL
  USING (auth.jwt() ->> 'role' = 'moderator');

CREATE POLICY "Moderators can update timestamp_triggers"
  ON timestamp_triggers FOR UPDATE
  USING (auth.jwt() ->> 'role' = 'moderator');
  `,
};

/**
 * Get all SQL schemas as a single migration string
 */
export function getAllSchemas(): string {
  return [
    SQL_SCHEMAS.media_content,
    SQL_SCHEMAS.overall_triggers,
    SQL_SCHEMAS.timestamp_triggers,
    SQL_SCHEMAS.timestamp_votes,
    SQL_SCHEMAS.rls_policies,
  ].join('\n\n-- ========================================\n\n');
}

/**
 * Helper to get severity column name for a category
 */
export function getSeverityColumnName(category: TriggerCategory): string {
  return `${category}_severity`;
}

/**
 * Type guard for valid categories
 */
export function isValidTriggerCategory(category: string): category is TriggerCategory {
  return CATEGORY_KEYS.includes(category as TriggerCategory);
}
