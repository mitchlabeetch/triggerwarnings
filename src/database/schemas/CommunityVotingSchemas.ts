...
export interface PatternSubmission {
  ...
  category: TriggerCategory;         // Which trigger category
  patternType: 'keyword' | 'phrase' | 'regex' | 'audio-signature' | 'visual-signature';
  patternData: string;               // The actual pattern
  ...
  weightedHelpfulScore: number;      // Bayesian weighted helpful score (0-100)
  weightedNotHelpfulScore: number;   // Bayesian weighted not helpful score (0-100)

  // Timestamps
  createdAt: Date;
  updatedAt: Date;
  reviewedAt?: Date;                 // When status changed to accepted/rejected

  // Performance tracking (after acceptance)
  timesMatched?: number;             // How many times pattern matched
  falsePositiveRate?: number;        // User-reported false positives
  truePositiveRate?: number;         // User-confirmed true positives
}
...
export interface Vote {
  ...
  // Vote weight (calculated by Bayesian engine)
  voteWeight: number;                // 0.5 - 2.0 (default 1.0)

  // Reasoning (optional)
  comment?: string;                  // Why this vote
...
export interface UserExpertise {
  ...
  // Per-category expertise scores (0-100)
  categoryExpertise: Record<TriggerCategory, number>;

  // Overall contribution stats
  totalContributions: number;        // Total patterns submitted
  acceptedContributions: number;     // Patterns accepted
  rejectedContributions: number;     // Patterns rejected

  // Voting accuracy
  totalVotes: number;                // Total votes cast
  consensusAgreementRate: number;    // % agreement with final consensus (0-1)
...
export interface PatternPerformance {
  ...
  // Match statistics
  totalMatches: number;              // How many times it matched
  uniqueVideos: number;              // Number of unique videos
  avgConfidence: number;             // Average confidence when matched

  // User feedback on matches
  userConfirmedMatches: number;      // Users said "yes, this was correct"
  userDismissedMatches: number;      // Users said "no, false positive"

  // Calculated metrics
  precision: number;                 // True positives / (true positives + false positives)
  falsePositiveRate: number;         // False positives / total matches
...
export interface PatternFeedback {
  ...
  timestamp: number;                 // When in video

  feedback: 'correct' | 'false-positive' | 'should-be-different-category';
  suggestedCategory?: TriggerCategory;  // If they think it's wrong category
  comment?: string;
 ...
 */
export function isValidPatternType(type: string): type is PatternSubmission['patternType'] {
  return ['keyword', 'phrase', 'regex', 'audio-signature', 'visual-signature'].includes(type);
}

export function isValidStatus(status: string): status is PatternSubmission['status'] {
  return ['pending', 'under-review', 'accepted', 'rejected', 'flagged'].includes(status);
}

export function isValidModality(modality: string): modality is PatternSubmission['modality'] {
