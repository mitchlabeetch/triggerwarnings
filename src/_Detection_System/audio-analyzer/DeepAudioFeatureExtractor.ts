...
export interface DeepAudioFeatures {
  ...
  mfccs: number[];  // Length: 13

  // Spectral features
  spectralContrast: number[];  // Length: 7 (frequency sub-bands)
  spectralCentroid: number;  // Single value (Hz)
  spectralBandwidth: number;  // Single value (Hz)
  spectralRolloff: number;  // Single value (Hz)

  // Chroma features (12 pitch classes: C, C#, D, D#, E, F, F#, G, G#, A, A#, B)
  chroma: number[];  // Length: 12

  // Time-domain features
  zeroCrossingRate: number;  // Single value (crossings per second)
  rmsEnergy: number;  // Single value (0-1)

  // Derived features
  spectralFlux: number;  // Change in spectrum over time
  spectralFlatness: number;  // Measure of noisiness (0 = tonal, 1 = noise)

  // Total: 13 + 7 + 1 + 1 + 1 + 12 + 1 + 1 + 1 + 1 = 39 features
}

/**
 * Category-specific audio feature importance
 */
interface CategoryFeatureWeights {
  mfccs: number;
...
export class DeepAudioFeatureExtractor {
  ...
  extractFeatures(audioData: Float32Array): DeepAudioFeatures {
    ...
    logger.debug(
      `[DeepAudioFeatureExtractor] Features extracted in ${extractionTime.toFixed(2)}ms | ` +
   ...
   */
  private extractMFCCs(audioData: Float32Array): number[] {
   ...
   */
  private extractSpectralContrast(audioData: Float32Array): number[] {
    const powerSpectrum = this.computePowerSpectrum(audioData);
   ...
   */
  private extractSpectralBandwidth(audioData: Float32Array, centroid: number): number {
    const powerSpectrum = this.computePowerSpectrum(audioData);
   ...
   */
  private extractSpectralRolloff(audioData: Float32Array, threshold: number = 0.85): number {
    const powerSpectrum = this.computePowerSpectrum(audioData);
   ...
   */
  private extractChroma(audioData: Float32Array): number[] {
    const powerSpectrum = this.computePowerSpectrum(audioData);
    ...
    for (let i = 0; i < powerSpectrum.length; i++) {
      const freq = (i * this.sampleRate) / this.fftSize;
      if (freq < 20) continue;  // Skip very low frequencies
   ...
   */
  private extractZeroCrossingRate(audioData: Float32Array): number {
    let crossings = 0;
   ...
   */
  private extractRMSEnergy(audioData: Float32Array): number {
    let sumSquares = 0;
   ...
   */
  private extractSpectralFlux(audioData: Float32Array): number {
   ...
   * - 1: white noise
   */
  private extractSpectralFlatness(audioData: Float32Array): number {
    const powerSpectrum = this.computePowerSpectrum(audioData);
   ...
   */
  private computePowerSpectrum(audioData: Float32Array): number[] {
   ...
   */
  private applyHammingWindow(audioData: Float32Array): Float32Array {
   ...
   */
  private getMelFilterbank(): number[][] {
