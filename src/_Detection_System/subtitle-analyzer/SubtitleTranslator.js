...
export class SubtitleTranslator {
    ...
    requestsToday = 0;
    dailyLimit = 9000; // Stay safely under 10k
    lastResetDate = '';
    ...
    async translateText(text, sourceLang) {
        ...
        try {
            ...
            if (data.responseStatus === 200 && data.responseData?.translatedText) {
                ...
                this.saveDailyCount();
                logger.debug(`Translated (${normalizedLang}→en): "${text.substring(0, 30)}..." → "${translated.substring(0, 30)}..." (confidence: ${confidence})`);
                return translated;
            }
            else {
     ...
     */
    async prefetchTranslations(cues, sourceLang, currentTime, prefetchWindow = 30) {
        // Get upcoming cues in prefetch window
        const upcomingCues = cues.filter((cue) => cue.startTime >= currentTime && cue.startTime <= currentTime + prefetchWindow);
        ...
        const translations = upcomingCues.map((cue) => this.translateText(cue.text, sourceLang).catch((error) => {
...
//# sourceMappingURL=SubtitleTranslator.js.map