...
export class VisualPrimaryPipeline {
    ...
    process(category, input, config) {
        ...
        const weightedConfidence = this.calculateWeightedConfidence(visualConfidence, audioConfidence, textConfidence, config.modalityWeights);
        ...
        logger.debug(`[VisualPrimaryPipeline] Processed ${category} | ` +
     ...
     */
    analyzeMedicalProcedure(features) {
        let confidence = 0;
     ...
     */
    analyzeFlashingLights(features) {
     ...
     */
    calculateWeightedConfidence(visual, audio, text, weights) {
 ...
 */
//# sourceMappingURL=VisualPrimaryPipeline.js.map