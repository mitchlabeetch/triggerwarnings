...
export class HierarchicalDetector {
    ...
    detect(input, timestamp) {
        ...
        if (!coarseResult.shouldContinue) {
            ...
            logger.debug(`[HierarchicalDetector] ⚡ EARLY EXIT (Stage 1) | ` +
        ...
        if (!mediumResult.shouldContinue) {
            ...
            this.updateAvgProcessingTime(totalTime);
            logger.debug(`[HierarchicalDetector] ⚡ EARLY EXIT (Stage 2) | ` +
        ...
        const totalTime = performance.now() - startTime;
        this.updateAvgProcessingTime(totalTime);
        logger.info(`[HierarchicalDetector] 🔍 FULL ANALYSIS (Stage 3) | ` +
     ...
     */
    coarseDetection(input) {
        const startTime = performance.now();
        const suspectedFamilies = new Set();
     ...
     */
    mediumRefinement(families, input) {
        const startTime = performance.now();
        const suspectedGroups = new Set();
     ...
     */
    fineDetection(groups, input) {
        const startTime = performance.now();
        const suspectedCategories = new Set();
     ...
     */
    shouldAnalyzeCategory(category, input) {
 ...
 */
//# sourceMappingURL=HierarchicalDetector.js.map