...
export interface AudioFeatures {
  waveformAnalysis: {
    transientDetected: boolean;    // Sudden spike (gunshot, explosion)
    transientIntensity: number;    // 0-100
    lowFrequencyRumble: number;    // 0-100 (explosions)
    impulseResponse: number;       // 0-100 (gunshot characteristic)
  };
  frequencyAnalysis: {
    peakFrequency: number;         // Hz
    harmonicDistortion: number;    // 0-100 (screams)
    spectralCentroid: number;      // Hz (brightness)
    zeroCrossingRate: number;      // Voice vs noise discrimination
  };
  intensityAnalysis: {
    peakAmplitude: number;         // 0-100
    suddenness: number;            // 0-100
...
export class AudioPrimaryPipeline {
  ...
  process(
    ...
    logger.debug(
      `[AudioPrimaryPipeline] Processed ${category} | ` +
   ...
   */
  private analyzeAudio(input: MultiModalInput, category: TriggerCategory): number {
    if (!input.audio) {
      return 0;
    }

    const features = input.audio.features as AudioFeatures | undefined;
    if (!features) {
   ...
   */
  private analyzeGunshot(features: AudioFeatures): number {
    let confidence = 0;
   ...
   */
  private analyzeExplosion(features: AudioFeatures): number {
    let confidence = 0;
   ...
   */
  private analyzeJumpscare(features: AudioFeatures): number {
    let confidence = 0;
   ...
   */
  private analyzeScream(features: AudioFeatures): number {
    let confidence = 0;
