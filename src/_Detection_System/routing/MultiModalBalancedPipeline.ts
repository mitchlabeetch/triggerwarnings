...
export interface CrossModalAgreement {
  visualAudioAgreement: boolean;   // Do visual and audio tell same story?
  visualTextAgreement: boolean;    // Do visual and text match?
  audioTextAgreement: boolean;     // Do audio and text match?
  overallAgreement: number;        // 0-100 score
}
...
export class MultiModalBalancedPipeline {
  ...
  process(
    ...
    const agreement = this.calculateCrossModalAgreement(
      visualConfidence,
      audioConfidence,
      textConfidence
    );
    ...
    const weightedConfidence = this.calculateBalancedConfidence(
      visualConfidence,
      audioConfidence,
      textConfidence,
    ...
    const validationPassed = this.validateHighSensitivityDetection(
      weightedConfidence,
      agreement,
    ...
    logger.debug(
      `[MultiModalBalancedPipeline] Processed ${category} | ` +
  ...
  private calculateCrossModalAgreement(
    ...
    const agreement: CrossModalAgreement = {
  ...
  private validateHighSensitivityDetection(
    ...
    allModalitiesPresent: boolean | undefined
  ): boolean {
    if (validationLevel === 'high-sensitivity') {
      ...
      if (agreement.overallAgreement < 60) {
        logger.debug(
          `[MultiModalBalancedPipeline] High-sensitivity validation FAILED: ` +
