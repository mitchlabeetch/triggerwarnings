...
function analyzeFrame(bitmap: ImageBitmap, timestamp: number) {
  if (!offscreenCanvas) {
...
function analyzeColors(imageData: ImageData): ColorAnalysis {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    // 1. BLOOD
    if (r > 200 && r > g + 50 && r > b + 50) brightRedCount++;

    // 2. FIRE
    if (r > 200 && g > 150 && b < 100) orangeYellowCount++;

    // 2b. VOMIT: Yellow-Brown
    if (r > 150 && g > 130 && r > b + 50 && g > b + 40 && b < 100) yellowBrownCount++;

    // 2c. VOMIT: Greenish-Yellow
    if (g > 150 && r > 120 && r < 200 && g > r && g > b + 30 && b < 120) greenishYellowCount++;

    // 3. STERILE WHITE
    if (luminance > 220 && Math.abs(r - g) < 20 && Math.abs(g - b) < 20) sterileWhiteCount++;

    // 4. MEDICAL BLUE-GREEN
    if (b > 150 && g > 150 && r < 120) medicalBlueGreenCount++;

    // 5. BLUE-GREEN TINT
    if (b > g && g > r && b > 100) blueGreenCount++;

    // 6. DARK PIXELS
    if (luminance < 50) darkPixelCount++;
...
function calculateIrregularity(imageData: ImageData): number {
  const data = imageData.data;
...
function calculateChunkiness(imageData: ImageData): number {
  const data = imageData.data;
  const width = imageData.width;
  const height = imageData.height;
...
function detectBlood(timestamp: number, analysis: ColorAnalysis) {
...
function detectGore(timestamp: number, analysis: ColorAnalysis) {
  if (analysis.brightRed > 0.10 && analysis.darkPixels > 0.25 && analysis.irregularity > 0.20) {
    ...
    if (!detectedEvents.has(eventKey)) {
      ...
      createWarning(eventKey, timestamp, 'gore', confidence,
...
function detectFire(timestamp: number, analysis: ColorAnalysis) {
...
function detectVomit(timestamp: number, analysis: ColorAnalysis) {
  // Yellow-brown
  if (analysis.yellowBrown > 0.12 && analysis.chunkiness > 0.15) {
    ...
    if (!detectedEvents.has(eventKey)) {
      ...
      createWarning(eventKey, timestamp, 'vomit', confidence,
...
function detectMedical(timestamp: number, analysis: ColorAnalysis) {
  if (analysis.sterileWhite > 0.30 && analysis.medicalBlueGreen > 0.15) {
    ...
    if (!detectedEvents.has(eventKey)) {
      ...
      createWarning(eventKey, timestamp, 'medical_procedures', confidence,
...
function detectUnderwater(timestamp: number, analysis: ColorAnalysis) {
  if (analysis.blueGreen > 0.40 && analysis.saturation < 0.4) {
    ...
    if (!detectedEvents.has(eventKey)) {
      ...
      createWarning(eventKey, timestamp, 'violence', 65,
  ...
  if (difference > 0.7) {
    ...
    if (sceneChangeCount > 10) {
      ...
      if (!detectedEvents.has(eventKey)) {
        ...
        createWarning(eventKey, timestamp, 'violence', 70,
          `Rapid scene changes detected (${sceneChangeCount} cuts in 5s) - may trigger anxiety`,
          null);
        sceneChangeCount = 0;
      }
    }
