...
export class IncrementalProcessor {
    ...
    // Performance targets
    TARGET_LATENCY_MS = 5;
    MAX_STREAM_AGE_MS = 30000; // 30 seconds
    constructor() {
     ...
     */
    processTextIncremental(text, sequenceId, position = 0) {
     ...
     */
    processVideoFrame(imageData, frameNumber, fps, sequenceId) {
        return this.processChunk({
            type: 'video-frame',
            data: {
                imageData,
     ...
     */
    processAudioChunk(samples, sampleRate, sequenceId, startTime = 0) {
        return this.processChunk({
            type: 'audio-chunk',
            data: {
                samples,
     ...
     */
    extractFeaturesIncremental(input, state) {
        const features = { completeness: 0 };
        switch (input.type) {
            case 'text':
                features.text = this.extractTextFeaturesIncremental(input.data, state);
                features.completeness = input.data.isComplete ? 100 : Math.min(95, state.chunkCount * 20);
                break;
            case 'video-frame':
                ...
                features.visual = this.extractVisualFeaturesIncremental(input.data, state);
            ...
            case 'audio-chunk':
                ...
                features.audio = this.extractAudioFeaturesIncremental(input.data, state);
                features.completeness = Math.min(100, state.chunkCount * 15);
                break;
            case 'image':
                features.visual = this.extractImageFeatures(input.data);
     ...
     */
    extractTextFeaturesIncremental(chunk, state) {
        const features = {};
     ...
     */
    extractVisualFeaturesIncremental(frame, state) {
        const features = {};
        ...
        if (state.accumulatedFeatures.visual?.colors) {
            const motion = this.estimateMotion(state.accumulatedFeatures.visual.colors, colorBins);
     ...
     */
    extractAudioFeaturesIncremental(chunk, state) {
        const features = {};
    ...
    mergeFeatures(accumulated, newFeatures) {
        ...
        // Update completeness
        accumulated.completeness = Math.max(accumulated.completeness || 0, newFeatures.completeness);
    }
    /**
     * Try to make early decision (lazy computation)
     */
    tryEarlyDecision(state) {
        ...
        const highConfidenceResults = quickResults.filter(r => r.confidence >= this.LAZY_THRESHOLDS.earlyStopThreshold);
     ...
     * Generate incremental results for all categories
     */
    generateIncrementalResults(state, inputType) {
        const results = [];
        const categories = [
     ...
     */
    predictFromPartialFeatures(category, features) {
        let confidence = 0;
        let modalities = 0;
        // Visual contribution
     ...
     */
    scoreVisualForCategory(category, visual) {
     ...
     */
    scoreAudioForCategory(category, audio) {
     ...
     */
    scoreTextForCategory(category, text) {
     ...
     */
    shouldSkipVisualProcessing(state) {
     ...
     */
    shouldSkipAudioProcessing(state) {
     ...
     */
    getHighestTextConfidence(state) {
        if (!state.accumulatedFeatures.text?.tokens)
            return 0;
     ...
     */
    getHighestVisualConfidence(state) {
        if (!state.accumulatedFeatures.visual?.colors)
            return 0;
     ...
     */
    estimateMotion(prevColors, currentColors) {
        const motion = [];
...
//# sourceMappingURL=IncrementalProcessor.js.map