...
export class CrossModalAttention {
     ...
     */
    computeAttention(category, features) {
        ...
        logger.debug(`[CrossModalAttention] ${category}: ${dominantPair.join('-')} correlation=${correlationScore.toFixed(2)}, boost=+${(crossModalBoost * 100).toFixed(1)}%`);
        return {
            category,
            fusedFeatures,
     ...
     */
    computePairwiseAttention(features, weights) {
     ...
     */
    applyAttention(query, key, weights) {
     ...
     */
    computeAlignmentScore(features1, features2) {
        // Cosine similarity
        let dotProduct = 0;
        let norm1 = 0;
        let norm2 = 0;
     ...
     */
    findStrongestCorrelation(pairwiseResults) {
        let maxScore = 0;
     ...
     * Compute cross-modal boost based on correlation
     */
    computeCrossModalBoost(correlationScore, category, modalities) {
        ...
        const matchingPattern = knownPatterns.find(p => p.modalities.sort().join('-') === modalities.sort().join('-'));
     ...
     */
    fuseWithAttention(features, weights, pairwiseResults) {
        const fused = new Array(this.FUSED_DIM).fill(0);
     ...
     */
    updateWeights(category, features, correct) {
     ...
     */
    updateWeightMatrix(matrix, direction) {
     ...
     */
    learnCorrelation(category, modalities, strength) {
        let patterns = this.correlationPatterns.get(category) || [];
        // Find existing pattern
        const existing = patterns.find(p => p.modalities.sort().join('-') === modalities.sort().join('-'));
        if (existing) {
     ...
     */
    getAvailableModalities(features) {
        const modalities = [];
        if (features.visual && features.visual.length > 0)
            modalities.push('visual');
        if (features.audio && features.audio.length > 0)
     ...
     */
    getSingleModalityResult(category, features, modality) {
     ...
     * Update statistics
     */
    updateStats(boost, correlationScore) {
