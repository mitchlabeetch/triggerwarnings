...
export class PersonalizedDetector {
    ...
    shouldWarn(detection, context) {
        ...
        let threshold = userSensitivityProfileManager.calculateThreshold(detection.category, context);
        const reasons = [];
        const adjustments = {};
        // Apply adaptive learning adjustments
        if (profile.advancedSettings.adaptiveLearning) {
            const adaptiveAdjustment = this.calculateAdaptiveAdjustment(detection.category, detection.confidence);
        ...
        logger.debug(`[PersonalizedDetector] ${detection.category} | ` +
    ...
    processFeedback(feedback) {
        ...
        if (history.length > 100) {
            this.feedbackHistory.set(feedback.category, history.slice(-100));
        }
        logger.info(`[PersonalizedDetector] Feedback recorded: ${feedback.category} | ` +
    ...
    generateInsights() {
        ...
        return {
            ...
            adaptiveLearningProgress: progress
        };
    }
}
/**
