...
export class WarningDeduplicator {
    ...
    constructor(config) {
        ...
        logger.info(`[TW WarningDeduplicator] ✅ Initialized | ` +
    ...
    processWarning(warning) {
        ...
        if (this.isRateLimited(warning)) {
            ...
            logger.warn(`[TW WarningDeduplicator] ⚠️ Rate limited: ${warning.categoryKey} ` +
        ...
        if (this.isTooSoonForCategory(warning)) {
            this.stats.duplicatesFiltered++;
            logger.debug(`[TW WarningDeduplicator] ⏭️ Too soon for ${warning.categoryKey} ` +
    ...
    findOrCreateGroup(warning) {
        ...
        if (!group) {
            ...
            logger.debug(`[TW WarningDeduplicator] 📦 New group created: ${key} at ${warning.startTime.toFixed(1)}s`);
        }
        return group;
    }
    /**
    ...
    addToGroup(group, warning) {
        ...
        logger.debug(`[TW WarningDeduplicator] ➕ Added to group | ` +
            `Category: ${warning.categoryKey} | ` +
     ...
     */
    extractSource(warning) {
        // Check ID for source hints
        if (warning.id.includes('subtitle'))
            return 'subtitle';
        if (warning.id.includes('audio-waveform'))
            return 'audio-waveform';
        if (warning.id.includes('audio-frequency'))
            return 'audio-frequency';
        if (warning.id.includes('visual'))
            return 'visual';
        if (warning.id.includes('photosensitivity'))
            return 'photosensitivity';
        if (warning.id.includes('temporal'))
            return 'temporal';
        if (warning.id.includes('fused'))
            return 'fusion';
        if (warning.id.includes('database'))
            return 'database';
        // Check submittedBy field
        if (warning.submittedBy) {
            if (warning.submittedBy.includes('subtitle'))
                return 'subtitle';
            if (warning.submittedBy.includes('audio'))
                return 'audio';
            if (warning.submittedBy.includes('visual'))
                return 'visual';
            if (warning.submittedBy.includes('photo'))
                return 'photosensitivity';
            if (warning.submittedBy.includes('fusion'))
                return 'fusion';
        }
        return null;
    }
    ...
    mergeWarnings(group) {
        ...
        const avgConfidence = Math.round(warnings.reduce((sum, w) => sum + w.confidenceLevel, 0) / warnings.length);
        // Use highest confidence as base
        const baseWarning = warnings.reduce((highest, current) => current.confidenceLevel > highest.confidenceLevel ? current : highest);
        ...
        this.outputWarnings = this.outputWarnings.filter(w => !warnings.some(gw => gw.id === w.id));
        ...
        logger.info(`[TW WarningDeduplicator] 🔀 MERGED WARNING | ` +
     ...
     */
    mergeDescriptions(warnings, sources) {
     ...
     */
    keepHighestConfidence(group, newWarning) {
        const currentHighest = group.warnings.reduce((highest, current) => current.confidenceLevel > highest.confidenceLevel ? current : highest);
        // If new warning is not the highest, suppress it
        if (newWarning.confidenceLevel < currentHighest.confidenceLevel) {
            this.stats.duplicatesFiltered++;
            logger.debug(`[TW WarningDeduplicator] ⏭️ Lower confidence suppressed: ` +
        ...
        if (newWarning.confidenceLevel > currentHighest.confidenceLevel) {
            ...
            this.stats.duplicatesFiltered++;
            this.stats.outputWarnings = this.outputWarnings.length;
            logger.info(`[TW WarningDeduplicator] ⬆️ Higher confidence replaces previous: ` +
                `${newWarning.confidenceLevel}% > ${currentHighest.confidenceLevel}%`);
            return newWarning;
        }
...
//# sourceMappingURL=WarningDeduplicator.js.map