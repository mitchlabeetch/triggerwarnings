...
export class ConfidenceFusionSystem {
  ...
  private readonly MIN_CONFIDENCE_THRESHOLD = 50;  // Don't fuse detections below 50%
  ...
  private findRelatedDetections(detection: Detection): Detection[] {
    return this.recentDetections.filter(d =>
      d !== detection &&  // Not the same detection
   ...
   */
  private calculateFusedConfidence(detections: Detection[]): number {
   ...
   */
  private getPriorProbability(category: TriggerCategory): number {
    // Base rates vary by category
    const priors: Partial<Record<TriggerCategory, number>> = {
   ...
   */
  private calculateTemporalConsistency(detections: Detection[]): number {
   ...
   */
  private isFalsePositive(detection: Detection): boolean {
    ...
    if (detection.source === 'subtitle') {
      ...
      const fpCategories: TriggerCategory[] = ['sex', 'violence'];
