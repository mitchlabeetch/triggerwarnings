...
export interface CombinedInput {
  ...
  timestamp: number;
  alignment: {
    audioTextSync: number;  // 0-1 score
...
export interface FeatureVectors {
  textFeatures: number[];    // e.g., 768-dim BERT embeddings
  audioFeatures: number[];   // e.g., 128-dim MFCCs
  visualFeatures: number[];  // e.g., 512-dim CNN features
}
...
export interface LatentVector {
  combined: number[];        // Unified representation
  dimensionality: number;
  confidence: number;        // How well modalities align
}
...
export interface FusedDetection extends Detection {
  ...
  latentConfidence: number;   // How well features aligned in latent space
}
...
export class HybridFusionPipeline {
  ...
  async processHybrid(
    ...
    logger.debug(
      `[HybridFusionPipeline] Processed ${category} | ` +
  ...
  earlyFusion(input: RawInput): CombinedInput {
    ...
    const audioTextSync = this.alignTimestamps(
      input.subtitleText,
      input.audioBuffer,
      input.timestamp
    );
    ...
    const audioVisualSync = this.checkAudioVisualSync(
      input.audioBuffer,
      input.visualFrame
    );

    // Check text-visual consistency (does text describe what's visible?)
    const textVisualSync = this.checkTextVisualConsistency(
      input.subtitleText,
      input.visualFrame
    );
   ...
   * Check audio-visual synchronization
   */
  private checkAudioVisualSync(audio: Float32Array, visual: ImageData): number {
   ...
   */
  private checkTextVisualConsistency(text: string, visual: ImageData): number {
   ...
   */
  private extractFeatures(input: CombinedInput): FeatureVectors {
   ...
   */
  private generatePlaceholderFeatures(dim: number): number[] {
  ...
  intermediateFusion(features: FeatureVectors): LatentVector {
    ...
    const confidence = this.calculateFeatureAlignment(
      textProjected,
      audioProjected,
      visualProjected
    );
   ...
   */
  private projectFeatures(features: number[], targetDim: number): number[] {
   ...
   */
  private calculateModalityAgreement(detections: Detection[]): number {
