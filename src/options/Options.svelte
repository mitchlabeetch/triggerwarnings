<script lang="ts">
    ...
    try {
      ...
      const fetchProfile = new Promise((resolve, reject) => {
        if (typeof chrome !== 'undefined' && chrome.runtime) {
            ...
            browser.runtime.sendMessage({ type: 'GET_ACTIVE_PROFILE' }).then(resolve).catch(() => resolve(null));
        }
      });
  ...
  function updateTheme(theme: Theme) { updateProfile({ theme }); }
  function updateLeadTime(time: number) { updateProfile({ leadTime: time }); }
  function toggleSound() { if(activeProfile) updateProfile({ soundEnabled: !activeProfile.soundEnabled }); }
  function updateDefaultProtection(p: string) { updateProfile({ defaultProtection: p }); }
  function updateHelperMode(enabled: boolean) { updateProfile({ helperMode: enabled }); }
  ...
  <div class="container">
    ...
    {:else if activeProfile}
      <div class="options-content">
        <!-- Tabs -->
        <nav class="tabs">
          <button class="tab" class:active={activeTab === 'categories'} on:click={() => activeTab = 'categories'}>Categories</button>
          <button class="tab" class:active={activeTab === 'settings'} on:click={() => activeTab = 'settings'}>Settings</button>
          <button class="tab" class:active={activeTab === 'stats'} on:click={() => activeTab = 'stats'}>Stats</button>
        </nav>
          ...
          <section class="section">
            ...
            <div class="categories-grid">
              {#each CATEGORY_KEYS as key}
                ...
                <button class="category-card" class:enabled={isCategoryEnabled(key)} on:click={() => toggleCategory(key)}>
        ...
        {#if activeTab === 'settings'}
          <section class="section">
            ...
            <div class="setting-group">
              ...
              <input type="range" min="5" max="60" value={activeProfile.leadTime} on:change={(e) => updateLeadTime(Number(e.currentTarget.value))} />
            </div>
          </section>
        {/if}
...
<style>
  .options { min-height: 100vh; background: #f5f7fa; font-family: system-ui, sans-serif; }
  .options-header { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px 0; }
  .container { max-width: 1000px; margin: 0 auto; padding: 0 20px; }
  .header-title { font-size: 24px; margin: 0; display: flex; align-items: center; gap: 10px; }
  .tabs { display: flex; gap: 20px; margin: 30px 0; border-bottom: 1px solid #ddd; }
  .tab { background: none; border: none; padding: 10px 0; cursor: pointer; font-size: 16px; color: #666; border-bottom: 2px solid transparent; }
  .tab.active { color: #667eea; border-bottom-color: #667eea; font-weight: 600; }
  .categories-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 15px; }
  .category-card { display: flex; align-items: center; gap: 15px; padding: 15px; background: white; border: 1px solid #ddd; border-radius: 8px; cursor: pointer; text-align: left; }
  .category-card.enabled { border-color: #667eea; background: #f8f9ff; }
  .category-icon { font-size: 24px; }
  .category-name { font-weight: 600; color: #333; }
  .category-toggle { font-weight: bold; color: #667eea; margin-left: auto; }
  .theme-selector { display: flex; gap: 10px; }
  .theme-btn { padding: 10px 20px; background: white; border: 1px solid #ddd; cursor: pointer; border-radius: 6px; }
