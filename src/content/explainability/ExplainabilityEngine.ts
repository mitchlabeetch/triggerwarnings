...
export interface InnovationContribution {
  phase: number;  // 1-5
  innovation: string;  // Name of innovation
  contribution: number;  // Confidence boost/adjustment (can be negative)
  reasoning: string;
}
...
export interface DetectionExplanation {
  ...
  threshold: number;        // User's threshold
  ...
  totalBoost: number;  // Sum of all boosts

  // Processing steps
  processingSteps: Array<{
    step: string;
    confidence: number;  // Confidence after this step
  ...
  // Visualization data
  confidenceTimeline: Array<{ step: string; confidence: number }>;
}
...
export class ExplainabilityEngine {
  ...
  private stats = {
    ...
    innovationTracking: new Map<string, number>()
  };
  ...
  explain(detectionData: {
    ...
    // Modality data
    modalityConfidences: { visual: number; audio: number; text: number };
    modalityWeights: { visual: number; audio: number; text: number };
    categoryFeatures?: any;

    // Innovation contributions
    hierarchicalTime?: number;
    featureExtraction?: { features: Record<string, number>; confidence: number };
    dependencyBoost?: { totalBoost: number; boosts: Array<{ fromCategory: string; boostAmount: number }> };
    multiTaskBoost?: { totalBoost: number; relatedCategories: string[] };
    temporalBoost?: number;
    validationPassed?: boolean;
    validationModalities?: number;
    adaptiveThreshold?: number;
    fewShotMatch?: { matched: boolean; label?: string; confidence?: number };

    // Processing steps
    processingSteps: Array<{ step: string; confidence: number; duration: number }>;
    ...
    logger.debug(
      `[ExplainabilityEngine] Explanation generated | ` +
   ...
   * Build modality breakdown
   */
  private buildModalityBreakdown(data: any): ModalityContribution[] {
    const breakdown: ModalityContribution[] = [];

    // Visual modality
    if (data.modalityConfidences.visual > 0) {
      const visualFeatures: Array<{ name: string; value: number; contribution: number }> = [];
      ...
      if (data.featureExtraction?.features) {
        for (const [name, value] of Object.entries(data.featureExtraction.features)) {
          visualFeatures.push({
            ...
            contribution: (value as number) * 0.1  // Simplified contribution
          });
        }
      }
   ...
   * Build innovation contributions
   */
  private buildInnovationContributions(data: any): InnovationContribution[] {
    const contributions: InnovationContribution[] = [];
   ...
   * Determine primary modality
   */
  private determinePrimaryModality(breakdown: ModalityContribution[]): 'visual' | 'audio' | 'text' | 'multi-modal' {
    if (breakdown.length === 0) return 'multi-modal';
    if (breakdown.length > 1) return 'multi-modal';
  ...
  private buildSummary(
    ...
    return (
      `${category} detected at ${data.timestamp.toFixed(1)}s with ${confidence}% confidence. ` +
  ...
  private buildDetailedReasoning(
    ...
    for (const modality of modalityBreakdown) {
      reasoning.push(
        `  ${modality.modality.toUpperCase()}: ${modality.confidence.toFixed(0)}% ` +
   ...
   * Update statistics
   */
  private updateStats(explanation: DetectionExplanation): void {
