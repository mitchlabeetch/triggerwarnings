...
import { SENSITIVITY_THRESHOLDS, DEFAULT_SENSITIVITY_PROFILE } from '@shared/types/UserSensitivityProfile.types';
...
export class UserSensitivityProfileManager {
    ...
    async loadProfile(userId) {
        ...
        try {
            const key = `sensitivity_profile_${userId}`; // Dynamic key
    ...
    async saveProfile(profile) {
        ...
        try {
            ...
            const key = `sensitivity_profile_${profile.userId}`; // Dynamic key
            await StorageAdapter.set(key, profile); // Cast to any because dynamic key is not in schema
