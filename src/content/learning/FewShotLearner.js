...
export class FewShotLearner {
    ...
    supportSet = new Map(); // id → example
    // Prototypes: one per few-shot pattern
    prototypes = new Map(); // label → prototype
    // Distance threshold for matching (tunable)
    DISTANCE_THRESHOLD = 0.3; // Euclidean distance threshold
    MIN_CONFIDENCE = 40; // Minimum confidence for few-shot match
    // Statistics
    stats = {
        totalExamples: 0,
        totalPrototypes: 0,
    ...
    addExample(example) {
        ...
        logger.info(`[FewShotLearner] Example added | ` +
    ...
    updatePrototype(example) {
        ...
        if (existingPrototype) {
            ...
            const newEmbedding = existingPrototype.embedding.map((val, i) => (val * n + example.features.combined[i]) / (n + 1));
            ...
            logger.debug(`[FewShotLearner] Prototype updated | ` +
                `${example.label} | ` +
            ...
            logger.info(`[FewShotLearner] 🆕 New prototype created | ` +
                `${example.label} | ` +
                `Category: ${example.category}`);
        }
    }
    ...
    predict(category, features) {
        ...
        const categoryPrototypes = Array.from(this.prototypes.values()).filter(p => p.category === category);
     ...
     */
    computeDistance(vector1, vector2) {
     ...
     */
    getPrototypesForCategory(category) {
    ...
    importData(data) {
        ...
        logger.info(`[FewShotLearner] Data imported | ` +
 ...
 */
//# sourceMappingURL=FewShotLearner.js.map