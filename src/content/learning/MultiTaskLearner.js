...
export class MultiTaskLearner {
    ...
    categoryToGroup = new Map();
    // Shared encoders (one per group)
    sharedEncoders = new Map();
    ...
    constructor() {
        ...
        for (const [group, categories] of Object.entries(this.CATEGORY_GROUPS)) {
            for (const category of categories) {
                this.categoryToGroup.set(category, group);
            }
        }
        // Initialize shared encoders with random weights
        for (const group of Object.keys(this.CATEGORY_GROUPS)) {
     ...
     */
    forwardSharedEncoder(group, features) {
     ...
     */
    forwardTaskHead(category, sharedFeatures) {
     ...
     */
    applyKnowledgeTransfer(targetCategory, group, sharedFeatures) {
        const boosts = [];
     ...
     */
    computeFeatureSimilarity(category1, category2, sharedFeatures) {
     ...
     */
    addTrainingSample(features, labels, timestamp) {
    ...
    trainMiniBatch() {
        ...
        for (const sample of batch) {
            for (const [category, label] of Object.entries(sample.labels)) {
                const prediction = this.predict(category, sample.features);
        ...
        logger.debug(`[MultiTaskLearner] Mini-batch training complete | ` +
     ...
     */
    updateAvgKnowledgeTransferBoost(newBoost) {
    ...
    exportWeights() {
        return {
            encoders: Object.fromEntries(Array.from(this.sharedEncoders.entries()).map(([group, encoder]) => [
                group,
                { weights: encoder.weights, bias: encoder.bias }
            ])),
            heads: Object.fromEntries(Array.from(this.taskHeads.entries()).map(([category, head]) => [
                category,
 ...
 */
//# sourceMappingURL=MultiTaskLearner.js.map