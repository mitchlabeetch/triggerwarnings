...
export class ModelEnsembleAggregator {
    ...
    stackingWeights = new Map();
    // Boosting weights (for hard examples)
    boostingWeights = new Map();
    // Model performance tracking
    modelPerformance = new Map();
    // Ensemble parameters
    VOTING_THRESHOLD = 0.5; // Majority vote threshold
    DIVERSITY_THRESHOLD = 0.3; // Min diversity for ensemble
     ...
     */
    aggregate(predictions, category) {
        ...
        const aggregationStrategy = this.selectAggregationStrategy(modelAgreement, diversityScore, predictions.length);
        // Step 6: Compute final ensemble confidence
        const ensembleConfidence = this.computeFinalConfidence(votingConfidence, stackingConfidence, boostingConfidence, aggregationStrategy);
        // Step 7: Extract model weights for this prediction
        const modelWeights = this.getModelWeightsForCategory(predictions.map(p => p.modelName), category);
        // Update statistics
        this.updateStats(ensembleConfidence, modelAgreement, diversityScore, aggregationStrategy);
        logger.debug(`[ModelEnsembleAggregator] ${category}: ensemble=${ensembleConfidence.toFixed(1)}%, ` +
     ...
     */
    weightedVoting(predictions, category) {
        let weightedSum = 0;
        let totalWeight = 0;
     ...
     */
    selectAggregationStrategy(modelAgreement, diversityScore, numModels) {
     ...
     */
    computeFinalConfidence(votingConf, stackingConf, boostingConf, strategy) {
        switch (strategy) {
            case 'voting':
                return votingConf;
            case 'stacking':
     ...
     */
    computeModelAgreement(predictions) {
        ...
        for (let i = 0; i < predictions.length; i++) {
            for (let j = i + 1; j < predictions.length; j++) {
                ...
                const agreement = 1 - Math.abs(predictions[i].confidence - predictions[j].confidence);
     ...
     */
    computeDiversityScore(predictions) {
        if (predictions.length < 2)
            return 0;
        // Diversity = variance in predictions
        const mean = predictions.reduce((sum, p) => sum + p.confidence, 0) / predictions.length;
        const variance = predictions.reduce((sum, p) => sum + Math.pow(p.confidence - mean, 2), 0) / predictions.length;
        // Normalize to [0, 1]
     ...
     */
    getModelWeight(modelName, category) {
     ...
     */
    getModelWeightsForCategory(modelNames, category) {
        const weights = {};
     ...
     */
    updateModelWeight(modelName, category, correct) {
        // Get current weight
     ...
     */
    updateBoostingWeight(modelName, handlesHardExamples) {
     ...
     */
    updateModelPerformance(modelName, correct) {
     ...
     * Update statistics
     */
    updateStats(ensembleConf, agreement, diversity, strategy) {
...
//# sourceMappingURL=ModelEnsembleAggregator.js.map