...
export class ActiveLearningSelector {
    ...
    UNCERTAINTY_THRESHOLD = 0.4; // Min uncertainty to consider
    COMMITTEE_DISAGREEMENT_MIN = 0.3;
    DIVERSITY_RADIUS = 0.1; // Min distance for diversity
    // Previously queried samples (to avoid duplicates)
    queriedSamples = new Set();
     ...
     */
    selectQueries(candidates, maxQueries = 5) {
        ...
        logger.info(`[ActiveLearningSelector] 📋 Selected ${selected.length} queries: ` +
     ...
     */
    queryByCommittee(candidate, committeePredictions) {
     ...
     */
    generateReason(candidate, strategy, score) {
     ...
     */
    estimateUncertainty(confidence) {
     ...
     */
    updateLabelEfficiency(selection, accuracyGain) {
        ...
        logger.debug(`[ActiveLearningSelector] 📈 Label efficiency: ` +
     ...
     */
    selectDiverseBatch(candidates, batchSize) {
        const selected = [];
            ...
            const scored = remaining.map(candidate => {
                ...
                if (selected.length > 0) {
                    ...
                    for (const sel of selected) {
                        const dist = this.euclideanDistance(candidate.features, sel.candidate.features);
...
//# sourceMappingURL=ActiveLearningSelector.js.map