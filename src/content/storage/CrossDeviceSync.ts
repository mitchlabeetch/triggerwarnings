...
export interface SyncConfig {
  ...
  uploadAnalytics: boolean;  // Opt-in for anonymous analytics
}
...
export class CrossDeviceSync {
  ...
  // Analytics buffer
  private analyticsBuffer: AnalyticsData | null = null;
   ...
   */
  async syncToBackend(snapshot: LearningStateSnapshot): Promise<boolean> {
    ...
    try {
      ...
      if (this.config.syncThresholds && snapshot.thresholds.length > 0) {
        ...
        if (success) itemsSynced += snapshot.thresholds.length;
      }
      ...
      if (this.config.syncFewShot && snapshot.fewShotExamples.length > 0) {
        ...
        if (success) itemsSynced += snapshot.fewShotExamples.length;
      }
   ...
   */
  async syncFromBackend(): Promise<LearningStateSnapshot | null> {
    if (!this.config.enabled || !this.supabase) {
      return null;
    }
   ...
   */
  private async syncThresholds(thresholds: ThresholdData[]): Promise<boolean> {
   ...
   */
  private async syncFewShotExamples(examples: FewShotExample[]): Promise<boolean> {
    if (!this.supabase) return false;
   ...
   */
  private async syncFeedbackSummary(feedback: any[]): Promise<boolean> {
    if (!this.supabase || feedback.length === 0) return false;
  ...
  collectAnalytics(data: Partial<AnalyticsData>): void {
    ...
    if (!this.analyticsBuffer) {
      this.analyticsBuffer = {
        ...
        periodEnd: Date.now(),
        totalDetections: 0,
        detectionsByCategory: {} as any,
        avgConfidenceByCategory: {} as any,
        totalFeedback: 0,
        feedbackByType: {},
        dismissRateByCategory: {} as any,
        falsePositiveRate: 0,
        falseNegativeRate: 0,
   ...
   */
  private async uploadAnalytics(data: AnalyticsData): Promise<boolean> {
    if (!this.supabase) return false;
   ...
   */
  private mergeSnapshots(local: LearningStateSnapshot, remote: LearningStateSnapshot): LearningStateSnapshot {
    // Use most recent version
    const version = local.savedAt > remote.savedAt ? local.version : remote.version;

    // Merge thresholds (prefer newer adjustments)
    const thresholdMap = new Map<TriggerCategory, ThresholdData>();
    ...
    const exampleMap = new Map<string, FewShotExample>();
...
export function initializeCrossDeviceSync(
  supabaseUrl: string,
  supabaseKey: string,
  userId: string,
