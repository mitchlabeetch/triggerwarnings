...
export interface MultiTaskWeights {
  sharedEncoders: Record<string, number[]>;  // Group -> weights
  taskHeads: Record<TriggerCategory, number[]>;  // Category -> weights
  knowledgeTransferMatrix: number[][];  // 28x28 matrix
  lastUpdated: number;
...
export interface FewShotExample {
  ...
  label: boolean;  // True = trigger, False = not trigger
...
export class ProgressiveLearningState {
  ...
  private readonly DB_NAME = 'TriggerWarningsLearning';
  ...
  // Configuration
  private readonly MAX_FEEDBACK_HISTORY = 10000;  // Keep last 10k feedback events
  private readonly MAX_FEW_SHOT_EXAMPLES = 1000;  // Keep best 1k examples per category
  private readonly AUTOSAVE_INTERVAL = 60000;     // Autosave every minute

  // In-memory cache (for fast access)
  private thresholdsCache: Map<TriggerCategory, ThresholdData> = new Map();
  private fewShotCache: Map<TriggerCategory, FewShotExample[]> = new Map();
  private feedbackCache: FeedbackHistory[] = [];
  ...
  constructor(userId: string, algorithmVersion: string = '3.0-phase-7') {
    this.userId = userId;
   ...
   */
  async saveThresholds(thresholds: ThresholdData[]): Promise<boolean> {
   ...
   */
  async loadThresholds(category?: TriggerCategory): Promise<ThresholdData[]> {
    // Check cache first
   ...
   */
  async saveMultiTaskWeights(weights: MultiTaskWeights): Promise<boolean> {
    if (!this.indexedDB) return false;
   ...
   */
  async loadMultiTaskWeights(): Promise<MultiTaskWeights | null> {
   ...
   */
  async saveFewShotExamples(examples: FewShotExample[]): Promise<boolean> {
    if (!this.indexedDB) return false;
   ...
   */
  async loadFewShotExamples(category?: TriggerCategory): Promise<FewShotExample[]> {
    // Check cache first
    ...
    try {
      ...
      if (category) {
        ...
        // Group by category
        const grouped = new Map<TriggerCategory, FewShotExample[]>();
        examples.forEach(ex => {
   ...
   */
  async pruneFewShotExamples(maxPerCategory: number = this.MAX_FEW_SHOT_EXAMPLES): Promise<void> {
    if (!this.indexedDB) return;
   ...
   * Save user feedback
   */
  async saveFeedback(feedback: FeedbackHistory[]): Promise<boolean> {
    if (!this.indexedDB) return false;
   ...
   */
  async loadFeedback(category?: TriggerCategory, limit: number = 1000): Promise<FeedbackHistory[]> {
    if (!this.indexedDB) return [];
  ...
  async restoreSnapshot(timestamp: number): Promise<boolean> {
    ...
    try {
      ...
      const snapshot: LearningStateSnapshot = request.result;
   ...
   */
  async loadAllState(): Promise<void> {
  ...
  async saveAllState(
    thresholds: ThresholdData[],
    multiTask: MultiTaskWeights | null,
   ...
   */
  private waitForRequest(request: IDBRequest): Promise<void> {
    return new Promise((resolve, reject) => {
...
// Singleton factory
let learningStateInstance: ProgressiveLearningState | null = null;

export function initializeProgressiveLearning(userId: string, version?: string): ProgressiveLearningState {
  learningStateInstance = new ProgressiveLearningState(userId, version);
  return learningStateInstance;
}

export function getProgressiveLearning(): ProgressiveLearningState | null {
  return learningStateInstance;
}
