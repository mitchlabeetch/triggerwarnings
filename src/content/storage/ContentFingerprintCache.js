...
export class ContentFingerprintCache {
    ...
    indexedDB = null;
    DB_NAME = 'TriggerWarningsCache';
    DB_VERSION = 1;
    STORE_NAME = 'detectionResults';
    // Configuration
    MAX_MEMORY_ENTRIES = 1000; // Max entries in memory
    DEFAULT_TTL = 30 * 24 * 60 * 60 * 1000; // 30 days
    SIMILARITY_THRESHOLD = 0.90; // 90% similar = match
    // Statistics
    stats = {
        totalRequests: 0,
        cacheHits: 0,
     ...
     */
    generateImageFingerprint(imageData, options = {}) {
     ...
     */
    generateAudioFingerprint(samples, sampleRate = 48000, options = {}) {
        try {
     ...
     */
    generateTextFingerprint(text, options = {}) {
        try {
     ...
     */
    async getCachedResults(fingerprint) {
     ...
     */
    async cacheResults(fingerprint, results, ttl = this.DEFAULT_TTL) {
        const entry = {
            fingerprint,
            results,
            cachedAt: Date.now(),
     ...
     */
    async invalidateFingerprint(fingerprint) {
        // Remove from L1
     ...
     */
    async getFromIndexedDB(fingerprint) {
        if (!this.indexedDB)
            return null;
     ...
     */
    async deleteFromIndexedDB(fingerprint) {
        if (!this.indexedDB)
            return;
        try {
            const transaction = this.indexedDB.transaction([this.STORE_NAME], 'readwrite');
     ...
     */
    downscaleImage(imageData, targetWidth, targetHeight) {
     ...
     * Downsample audio
     */
    downsampleAudio(samples, fromRate, toRate) {
        if (fromRate === toRate)
            return samples;
...
//# sourceMappingURL=ContentFingerprintCache.js.map