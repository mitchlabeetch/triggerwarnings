...
export type DetectionFeedbackType =
  ...
  | 'reported_missed'     // User reported algorithm missed a trigger
  | 'severity_too_high'   // Warning was too severe
  | 'severity_too_low'    // Warning wasn't severe enough
  | 'confirmed_accurate'  // User confirmed it was accurate
  | 'false_positive';     // User says it's wrong

/**
 * Feedback types for community triggers
 */
export type CommunityTriggerVote =
  | 'helpful'          // This trigger is helpful
  | 'not_helpful'      // This trigger is not helpful
  | 'false_positive'   // This trigger matches when it shouldn't
  | 'confirmed';       // This trigger correctly identified content
...
export interface DetectionFeedback {
  ...
  userId: string;
  userComment?: string;
  severityAdjustment?: number; // -2 to +2
  providedAt?: Date;
}
...
export interface TriggerPerformance {
  ...
  falsePositiveCount: number;
  missedCount: number;
  consensusScore: number; // 0-1 (1 = everyone agrees)
}
...
/**
 * Supabase client interface (simplified - use real @supabase/supabase-js in production)
 */
...
export class UnifiedContributionPipeline {
  ...
  private supabase: any; // SupabaseClient (will be initialized)
  private userId: string;
  private sessionId: string;

  // Promotion criteria (gaming-resistant)
  private readonly PROMOTION_CRITERIA: PromotionCriteria = {
  ...
  private approvedTriggersCache: Map<TriggerCategory, CommunityTrigger[]> = new Map();
   ...
   */
  async logDetection(detection: AlgorithmDetection): Promise<string | null> {
    if (!this.supabase) {
   ...
   */
  async logDetectionBatch(detections: AlgorithmDetection[]): Promise<number> {
    if (!this.supabase || detections.length === 0) return 0;
   ...
   */
  async submitCommunityTrigger(trigger: CommunityTrigger): Promise<string | null> {
    if (!this.supabase) {
   ...
   */
  private async recalculateValidationScore(triggerId: string): Promise<void> {
    if (!this.supabase) return;
    ...
    try {
      ...
      const confirmed = votes.filter((v: any) => v.vote_type === 'confirmed').length;
      const notHelpful = votes.filter((v: any) => v.vote_type === 'not_helpful').length;
      const falsePositive = votes.filter((v: any) => v.vote_type === 'false_positive').length;
   ...
   */
  private async checkTriggerPromotion(triggerId: string): Promise<void> {
    if (!this.supabase) return;
    ...
    try {
      ...
      if (
        voteCount >= this.PROMOTION_CRITERIA.minVotes &&
   ...
   */
  private async promoteTrigger(trigger: CommunityTrigger): Promise<void> {
    if (!this.supabase) return;
    ...
    try {
      ...
      logger.info(
        `[UnifiedPipeline] 🎉 TRIGGER PROMOTED! ` +
   ...
   */
  async getContentPerformance(contentFingerprint: string): Promise<TriggerPerformance[]> {
    if (!this.supabase) return [];
    ...
    try {
      ...
      if (data && data.length > 0) {
        const avgConsensus = data.reduce((sum: number, d: any) => sum + d.consensus_score, 0) / data.length;
   ...
   */
  async getApprovedTriggers(category?: TriggerCategory): Promise<CommunityTrigger[]> {
    // Check cache first
   ...
   */
  async preloadApprovedTriggers(): Promise<void> {
    ...
    await Promise.all(
      categories.map(category => this.getApprovedTriggers(category))
    );
...
let pipelineInstance: UnifiedContributionPipeline | null = null;

export function initializeUnifiedPipeline(
  supabaseUrl: string,
  supabaseKey: string,
  userId: string
): UnifiedContributionPipeline {
  pipelineInstance = new UnifiedContributionPipeline(supabaseUrl, supabaseKey, userId);
  return pipelineInstance;
}

export function getUnifiedPipeline(): UnifiedContributionPipeline | null {
  return pipelineInstance;
}
