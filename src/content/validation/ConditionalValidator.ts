 ...
 */
export const CATEGORY_VALIDATION_LEVEL: Record<TriggerCategory, ValidationLevel> = {
 ...
 */
export const VALIDATION_THRESHOLDS: Record<ValidationLevel, number> = {
...
export class ConditionalValidator {
  ...
  validate(detection: Detection, relatedDetections?: Detection[]): ValidationResult {
    ...
    if (result.passed) {
      logger.info(
        `[ConditionalValidator] ✅ VALIDATION PASSED | ` +
        `${detection.category} | ` +
      ...
      logger.info(
        `[ConditionalValidator] ❌ VALIDATION FAILED | ` +
        `${detection.category} | ` +
        `Level: ${validationLevel} | ` +
  ...
  private validateHighSensitivity(
    ...
    if (modalitiesPresent < modalitiesRequired) {
      ...
      reasoning.push(
        `❌ HIGH-SENSITIVITY REJECTION: Only ${modalitiesPresent} modality present, ` +
    ...
    reasoning.push(
      `✅ HIGH-SENSITIVITY PASSED: ${modalitiesPresent} modalities confirm detection ` +
  ...
  private validateStandard(
    ...
    if (modalitiesPresent === 1) {
      ...
      reasoning.push(
        `⚠️  STANDARD: Single modality only → reduced confidence by 40% ` +
      ...
      reasoning.push(
        `✅ STANDARD: ${modalitiesPresent} modalities confirm → confidence maintained/boosted`
      );
    }
    ...
    if (!passed) {
      reasoning.push(
        `❌ STANDARD FAILED: Adjusted confidence ${adjustedConfidence.toFixed(1)}% < threshold ${threshold}%`
      );
    }
  ...
  private validateSingleModality(
    ...
    reasoning.push(
      `✅ SINGLE-MODALITY-SUFFICIENT: ${modalitiesPresent} modality(s) detected | ` +
    ...
    if (!passed) {
      reasoning.push(
        `❌ SINGLE-MODALITY FAILED: Confidence ${adjustedConfidence.toFixed(1)}% < threshold ${threshold}%`
      );
    }
   ...
   */
  private countModalitiesPresent(detection: Detection, relatedDetections?: Detection[]): number {
    let count = 0;

    if (detection.modalityContributions) {
      if (detection.modalityContributions.visual > 10) count++;
      if (detection.modalityContributions.audio > 10) count++;
      if (detection.modalityContributions.text > 10) count++;
    } else {
   ...
   */
  private getModalityBreakdown(detection: Detection, relatedDetections?: Detection[]): Record<string, number> {
    if (detection.modalityContributions) {
      return {
    ...
    if (relatedDetections && relatedDetections.length > 0) {
      for (const det of relatedDetections) {
        if (det.source === 'visual') breakdown.visual = Math.max(breakdown.visual, det.confidence);
        if (det.source === 'audio') breakdown.audio = Math.max(breakdown.audio, det.confidence);
        if (det.source === 'text') breakdown.text = Math.max(breakdown.text, det.confidence);
      }
    } else {
      // Single detection - assign full confidence to source
      if (detection.source === 'visual') breakdown.visual = detection.confidence;
      else if (detection.source === 'audio') breakdown.audio = detection.confidence;
      else if (detection.source === 'text') breakdown.text = detection.confidence;
      else breakdown.visual = detection.confidence; // multi-modal fallback
    }
