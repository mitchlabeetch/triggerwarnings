...
export interface DetectionTask {
  id: string;
  category?: TriggerCategory; // Specific category or all
  input: ParallelInput;
...
export interface EngineStats {
  ...
  // Performance gain
  throughputMultiplier: number; // vs serial processing
...
interface QueuedTask {
  task: DetectionTask;
  promise: {
    resolve: (results: ParallelDetectionResult[]) => void;
...
export class ParallelDetectionEngine {
  ...
  // Throughput tracking
  private tasksInLastSecond: number = 0;
  private lastSecondTimestamp: number = Date.now();

  // Performance tracking
  private startTime: number = Date.now();
  private serialProcessingTimeMs: number = 0; // Estimated serial time
   ...
   */
  async detectAllCategories(input: ParallelInput, priority: TaskPriority = 'normal'): Promise<ParallelDetectionResult[]> {
   ...
   */
  private enqueueTask(task: DetectionTask): Promise<ParallelDetectionResult[]> {
   ...
   */
  private async processTaskOnWorker(worker: WorkerStats, queuedTask: QueuedTask): Promise<void> {
    ...
    try {
      ...
      const results: ParallelDetectionResult[] = [];
   ...
   */
  private async simulateDetection(category: TriggerCategory, input: ParallelInput): Promise<number> {
   ...
   */
  private scoreVisual(category: TriggerCategory, visual: ImageData): number {
   ...
   */
  private scoreAudio(category: TriggerCategory, audio: Float32Array): number {
   ...
   */
  private scoreText(category: TriggerCategory, text: string): number {
   ...
   */
  private updateStats(latencyMs: number, categoryCount: number): void {
    // Update task count
  ...
  private startPerformanceMonitoring(): void {
    setInterval(() => {
      ...
      this.stats.peakTasksPerSecond = Math.max(
        this.stats.peakTasksPerSecond,
        this.tasksInLastSecond
      );
  ...
  setWorkerCount(count: number): void {
    ...
    if (newCount > this.workerCount) {
      ...
      if (idleWorkers.length >= toRemove) {
        this.workers = this.workers.filter((w, idx) =>
          idx < newCount || w.status !== 'idle'
        ).slice(0, newCount);
