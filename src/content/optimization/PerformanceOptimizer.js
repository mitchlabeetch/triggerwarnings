...
export class PerformanceOptimizer {
    ...
    MEMORY_HIGH_THRESHOLD = 80; // % memory usage
    BATTERY_LOW_THRESHOLD = 20; // % battery
    constructor() {
        this.deviceCapabilities = this.detectDeviceCapabilities();
        this.currentMode = this.determineOptimalMode();
        logger.info(`[TW PerformanceOptimizer] ✅ Initialized | ` +
     ...
     */
    downgradeMode() {
        const modes = ['ULTRA', 'HIGH', 'MEDIUM', 'LOW', 'BATTERY_SAVER'];
        const currentIndex = modes.indexOf(this.currentMode);
        if (currentIndex < modes.length - 1) {
            const newMode = modes[currentIndex + 1];
            logger.warn(`[TW PerformanceOptimizer] ⚠️ CPU overloaded - downgrading from ${this.currentMode} to ${newMode}`);
    ...
    upgradeMode() {
        ...
        if (currentIndex > 0) {
            const newMode = modes[currentIndex - 1];
            logger.info(`[TW PerformanceOptimizer] ✅ CPU available - upgrading from ${this.currentMode} to ${newMode}`);
            this.setMode(newMode);
        }
    }
    /**
