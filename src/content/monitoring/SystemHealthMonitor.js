...
export class SystemHealthMonitor {
    ...
    constructor(config) {
        ...
        logger.info(`[TW SystemHealthMonitor] ✅ Initialized | ` +
     ...
     */
    registerSystem(name, healthCheck, restart) {
        const health = {
            name,
            status: 'healthy',
            uptime: 0,
            ...
            try {
                ...
                if (this.config.enableMemoryMonitoring && result.memoryUsage) {
                    ...
                    if (result.memoryUsage > this.config.memoryThreshold) {
                        logger.warn(`[TW SystemHealthMonitor] 💾 High memory usage: ${name} (${result.memoryUsage}MB)`);
                ...
                this.handleHealthCheckFail(system, error);
            }
        }
     ...
     */
    handleHealthCheckFail(system, error) {
        const now = Date.now();
        // Record error
        ...
        logger.error(`[TW SystemHealthMonitor] ❌ Health check failed: ${system.name} | ` +
     ...
     */
    async markSystemFailed(system) {
        system.health.status = 'failed';
        logger.error(`[TW SystemHealthMonitor] 💀 System FAILED: ${system.name} | ` +
            `Consecutive Failures: ${system.health.consecutiveFailures}`);
        ...
        if (this.config.autoRestart && system.restart) {
            ...
            logger.warn(`[TW SystemHealthMonitor] ⚠️ Auto-restart disabled or no restart function for ${system.name}`);
        }
    }
    /**
     * Degrade system status
     */
    degradeSystem(system, reason) {
    ...
    checkOverallHealth() {
        ...
        if (failedCount >= 2) {
            logger.error(`[TW SystemHealthMonitor] 💀 CASCADE FAILURE DETECTED | ` +
     ...
     */
    generateRecommendations(systems) {
        const recommendations = [];
        // Check for failed systems
        const failed = systems.filter(s => s.status === 'failed');
        if (failed.length > 0) {
            recommendations.push(`⚠️ CRITICAL: ${failed.length} system(s) failed: ${failed.map(s => s.name).join(', ')}`);
        }
        // Check for high error rates
        const highErrorRate = systems.filter(s => s.errorRate > this.config.errorThreshold);
        if (highErrorRate.length > 0) {
            recommendations.push(`⚠️ High error rate detected in: ${highErrorRate.map(s => s.name).join(', ')}`);
        }
        // Check for high memory usage
        const highMemory = systems.filter(s => s.memoryUsage !== null && s.memoryUsage > this.config.memoryThreshold);
        if (highMemory.length > 0) {
            recommendations.push(`💾 High memory usage: ${highMemory.map(s => `${s.name} (${s.memoryUsage}MB)`).join(', ')}`);
        }
        // Check for frequent restarts
        const frequentRestarts = systems.filter(s => s.restartCount >= 3);
        if (frequentRestarts.length > 0) {
            recommendations.push(`🔄 Frequent restarts: ${frequentRestarts.map(s => `${s.name} (${s.restartCount}x)`).join(', ')}`);
        }
    ...
    logHealthReport() {
        ...
        for (const system of report.systems) {
            ...
            logger.info(`${icon} ${system.name}: ${system.status.toUpperCase()} | ` +
