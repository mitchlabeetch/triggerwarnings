...
export class CategoryDependencyGraph {
     ...
     */
    addEdge(from, to, weight, reasoning) {
        const edge = { from, to, weight, reasoning };
     ...
     * related categories.
     */
    analyzeWithContext(detection, recentDetections) {
        ...
        const relevantDetections = detections.filter(d => Math.abs(d.timestamp - detection.timestamp) <= this.DETECTION_WINDOW &&
        ...
        logger.debug(`[CategoryDependencyGraph] ${detection.category} | ` +
     ...
     */
    getReverseDependencies(category) {
        const reverseDeps = [];
     ...
     */
    getStronglyRelatedCategories(category) {
 ...
 */
//# sourceMappingURL=CategoryDependencyGraph.js.map