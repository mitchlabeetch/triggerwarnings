...
export class VisualColorAnalyzer {
    ...
    previousFrame = null;
    sceneChangeCount = 0;
    sceneChangeWindow = 5000; // 5 seconds
    // Detection state
    ...
    initialize(videoElement) {
        ...
        logger.info(`[TW VisualColorAnalyzer] ✅ Initialized | ` +
     ...
     */
    calculateIrregularity(imageData) {
        const data = imageData.data;
    ...
    detectBlood(timestamp, analysis) {
        if (analysis.brightRed > 0.15) { // 15% of frame is bright red
            ...
            if (!this.detectedEvents.has(eventKey)) {
                ...
                logger.warn(`[TW VisualColorAnalyzer] 🩸 BLOOD DETECTED at ${timestamp.toFixed(1)}s | ` +
                    `Red pixels: ${(analysis.brightRed * 100).toFixed(1)}% | ` +
                    `Confidence: ${confidence}%`);
                this.createWarning(eventKey, timestamp, 'blood', confidence, `Blood detected via color analysis (${(analysis.brightRed * 100).toFixed(1)}% red pixels)`, analysis);
            }
        }
    }
    /**
    ...
    detectGore(timestamp, analysis) {
        ...
        if (analysis.brightRed > 0.10 &&
            ...
            if (!this.detectedEvents.has(eventKey)) {
                ...
                logger.warn(`[TW VisualColorAnalyzer] 💀 GORE DETECTED at ${timestamp.toFixed(1)}s | ` +
                    `Red: ${(analysis.brightRed * 100).toFixed(1)}%, ` +
                    `Irregularity: ${(analysis.irregularity * 100).toFixed(1)}% | ` +
                    `Confidence: ${confidence}%`);
                this.createWarning(eventKey, timestamp, 'gore', confidence, 'Gore detected via color analysis (red + shadows + irregular patterns)', analysis);
            }
        }
    }
    /**
    ...
    detectFire(timestamp, analysis) {
        if (analysis.orangeYellow > 0.20 && analysis.brightness > 0.6) {
            ...
            if (!this.detectedEvents.has(eventKey)) {
                ...
                logger.info(`[TW VisualColorAnalyzer] 🔥 FIRE DETECTED at ${timestamp.toFixed(1)}s | ` +
                    `Orange/Yellow: ${(analysis.orangeYellow * 100).toFixed(1)}% | ` +
                    `Confidence: ${confidence}%`);
                this.createWarning(eventKey, timestamp, 'violence', confidence, `Fire detected via color analysis (${(analysis.orangeYellow * 100).toFixed(1)}% orange/yellow)`, analysis);
            }
        }
    }
    /**
    ...
    detectVomit(timestamp, analysis) {
        ...
        if (analysis.yellowBrown > 0.12 && analysis.chunkiness > 0.15) {
            ...
            if (!this.detectedEvents.has(eventKey)) {
                ...
                logger.warn(`[TW VisualColorAnalyzer] 🤮 VOMIT DETECTED (yellow-brown) at ${timestamp.toFixed(1)}s | ` +
                    `Yellow-brown pixels: ${(analysis.yellowBrown * 100).toFixed(1)}% | ` +
                    `Chunkiness: ${(analysis.chunkiness * 100).toFixed(1)}% | ` +
                    `Confidence: ${confidence}%`);
                this.createWarning(eventKey, timestamp, 'vomit', confidence, `Vomit detected via color analysis (${(analysis.yellowBrown * 100).toFixed(1)}% yellow-brown, chunky texture)`, analysis);
            }
        }
        ...
        if (analysis.greenishYellow > 0.12 && analysis.chunkiness > 0.15) {
            ...
            if (!this.detectedEvents.has(eventKey)) {
                ...
                logger.warn(`[TW VisualColorAnalyzer] 🤮 VOMIT DETECTED (greenish-bile) at ${timestamp.toFixed(1)}s | ` +
                    `Greenish-yellow pixels: ${(analysis.greenishYellow * 100).toFixed(1)}% | ` +
                    `Chunkiness: ${(analysis.chunkiness * 100).toFixed(1)}% | ` +
                    `Confidence: ${confidence}%`);
                this.createWarning(eventKey, timestamp, 'vomit', confidence, `Vomit detected via color analysis (${(analysis.greenishYellow * 100).toFixed(1)}% greenish-bile, chunky texture)`, analysis);
            }
        }
        ...
        if (analysis.yellowBrown > 0.08 && analysis.greenishYellow > 0.08 && analysis.chunkiness > 0.12) {
            ...
            if (!this.detectedEvents.has(eventKey)) {
                ...
                logger.warn(`[TW VisualColorAnalyzer] 🤮 VOMIT DETECTED (mixed colors) at ${timestamp.toFixed(1)}s | ` +
                    `Yellow-brown: ${(analysis.yellowBrown * 100).toFixed(1)}%, ` +
                    `Greenish: ${(analysis.greenishYellow * 100).toFixed(1)}%, ` +
                    `Chunkiness: ${(analysis.chunkiness * 100).toFixed(1)}% | ` +
                    `Confidence: ${confidence}%`);
                this.createWarning(eventKey, timestamp, 'vomit', confidence, 'Vomit detected via color analysis (mixed yellow-brown and greenish colors, chunky texture)', analysis);
            }
        }
    }
    /**
    ...
    detectMedical(timestamp, analysis) {
        if (analysis.sterileWhite > 0.30 && analysis.medicalBlueGreen > 0.15) {
            ...
            if (!this.detectedEvents.has(eventKey)) {
                ...
                logger.debug(`[TW VisualColorAnalyzer] 🏥 MEDICAL SCENE DETECTED at ${timestamp.toFixed(1)}s | ` +
                    `Confidence: ${confidence}%`);
                this.createWarning(eventKey, timestamp, 'medical_procedures', confidence, 'Medical scene detected via color analysis (sterile environment)', analysis);
            }
        }
    }
    /**
    ...
    detectUnderwater(timestamp, analysis) {
        if (analysis.blueGreen > 0.40 && analysis.saturation < 0.4) {
            ...
            if (!this.detectedEvents.has(eventKey)) {
                ...
                logger.debug(`[TW VisualColorAnalyzer] 🌊 UNDERWATER SCENE DETECTED at ${timestamp.toFixed(1)}s`);
                // Underwater scenes can trigger drowning/suffocation fears
                this.createWarning(eventKey, timestamp, 'violence', 65, 'Underwater scene detected (potential drowning/suffocation trigger)', analysis);
            }
        }
    }
    /**
     ...
     */
    detectSceneChange(timestamp, currentFrame, previousFrame) {
        ...
        if (difference > 0.7) {
            ...
            if (this.sceneChangeCount > 10) {
                ...
                if (!this.detectedEvents.has(eventKey)) {
                    ...
                    logger.info(`[TW VisualColorAnalyzer] 🎬 RAPID SCENE CHANGES DETECTED at ${timestamp.toFixed(1)}s | ` +
                        `${this.sceneChangeCount} changes in 5 seconds`);
                    this.createWarning(eventKey, timestamp, 'violence', 70, `Rapid scene changes detected (${this.sceneChangeCount} cuts in 5s) - may trigger anxiety`, null);
     ...
     */
    createWarning(eventKey, timestamp, category, confidence, description, analysis) {
        const event = {
            type: category,
            timestamp,
            confidence,
