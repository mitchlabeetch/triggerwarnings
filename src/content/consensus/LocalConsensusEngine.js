...
export class LocalConsensusEngine {
     ...
     */
    async recordVote(videoId, categoryKey, timestamp, vote) {
        const newVote = {
            videoId,
            categoryKey,
            timestamp,
        ...
        const duplicateIndex = this.votes.findIndex(v => v.videoId === videoId &&
     ...
     */
    getThresholdAdjustment(categoryKey) {
     ...
     */
    getPreviousVote(videoId, categoryKey, timestamp) {
        const vote = this.votes.find(v => v.videoId === videoId &&
            v.categoryKey === categoryKey &&
...
//# sourceMappingURL=LocalConsensusEngine.js.map