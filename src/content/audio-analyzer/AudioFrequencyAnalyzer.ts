...
export class AudioFrequencyAnalyzer {
  ...
  private frequencyHistory: FrequencyBands[] = [];
  private readonly HISTORY_SIZE = 10;  // Keep last 1 second (100ms intervals)

  // Detection state
  private detectedEvents: Map<string, FrequencyEvent> = new Map();
   ...
   */
  private getAverageFrequency(minHz: number, maxHz: number, sampleRate: number): number {
   ...
   */
  private detectScream(timestamp: number, bands: FrequencyBands): void {
      ...
      if (this.isSustained('highMid', 180, 5)) {  // 5 checks = 500ms
        ...
        if (!this.detectedEvents.has(eventKey)) {
          ...
          const confidence = Math.min(
            Math.round(((bands.highMid + bands.presence) / 400) * 100),
            95
          );

          logger.warn(
            `[TW AudioFrequencyAnalyzer] 😱 SCREAM DETECTED at ${timestamp.toFixed(1)}s | ` +
            `Confidence: ${confidence}% | ` +
            `HighMid: ${bands.highMid.toFixed(0)}, Presence: ${bands.presence.toFixed(0)}`
          );

          this.createWarning(eventKey, timestamp, 'children_screaming', confidence,
            'Screaming detected via frequency analysis (3-5kHz sustained)');
        }
      }
    }
  }
   ...
   */
  private detectGunshot(timestamp: number, bands: FrequencyBands): void {
    ...
    if (this.frequencyHistory.length >= 2) {
      ...
      if (midIncrease > 100 && highMidIncrease > 100 &&
        ...
        if (!this.detectedEvents.has(eventKey)) {
          ...
          const confidence = Math.min(
            Math.round(((midIncrease + highMidIncrease) / 200) * 100),
            90
          );

          logger.warn(
            `[TW AudioFrequencyAnalyzer] 🔫 GUNSHOT DETECTED (freq) at ${timestamp.toFixed(1)}s | ` +
            `Confidence: ${confidence}%`
          );

          this.createWarning(eventKey, timestamp, 'violence', confidence,
            'Gunshot detected via frequency analysis (1-5kHz transient)');
        }
      }
    }
  }
   ...
   */
  private detectExplosion(timestamp: number, bands: FrequencyBands): void {
    ...
    if (bands.bass > 180 && bands.subBass > 150 && bands.mid > 120) {
      ...
      if (!this.detectedEvents.has(eventKey)) {
        ...
        const confidence = Math.min(
          Math.round(((bands.bass + bands.subBass) / 330) * 100),
          88
        );

        logger.warn(
          `[TW AudioFrequencyAnalyzer] 💥 EXPLOSION DETECTED (freq) at ${timestamp.toFixed(1)}s | ` +
          `Confidence: ${confidence}% | ` +
          `Bass: ${bands.bass.toFixed(0)}, SubBass: ${bands.subBass.toFixed(0)}`
        );

        this.createWarning(eventKey, timestamp, 'detonations_bombs', confidence,
          'Explosion detected via frequency analysis (20-200Hz surge)');
      }
    }
  }
   ...
   */
  private detectSiren(timestamp: number, bands: FrequencyBands): void {
    ...
    if (this.detectOscillation('mid', 2)) {  // 2 complete cycles
      ...
      if (!this.detectedEvents.has(eventKey)) {
        ...
        logger.info(
          `[TW AudioFrequencyAnalyzer] 🚨 SIREN DETECTED at ${timestamp.toFixed(1)}s`
        );

        this.createWarning(eventKey, timestamp, 'violence', 75,
          'Emergency siren detected via frequency analysis (oscillating 1-3kHz)');
      }
    }
  }
   ...
   */
  private detectMedicalBeep(timestamp: number, bands: FrequencyBands): void {
    ...
    if (this.detectPeriodicBeep('mid', 140, 5)) {
      ...
      if (!this.detectedEvents.has(eventKey)) {
        ...
        logger.debug(
          `[TW AudioFrequencyAnalyzer] 🏥 MEDICAL BEEP DETECTED at ${timestamp.toFixed(1)}s`
        );

        this.createWarning(eventKey, timestamp, 'medical_procedures', 70,
          'Medical equipment detected via frequency analysis (periodic 1-2kHz)');
      }
    }
  }
   ...
   */
  private detectCrying(timestamp: number, bands: FrequencyBands): void {
    ...
    if (bands.mid > 120 && bands.mid < 180 && this.isFluctuating('mid', 30, 5)) {
      ...
      if (!this.detectedEvents.has(eventKey)) {
        ...
        logger.debug(
          `[TW AudioFrequencyAnalyzer] 😢 CRYING DETECTED at ${timestamp.toFixed(1)}s`
        );

        this.createWarning(eventKey, timestamp, 'children_screaming', 65,
          'Crying detected via frequency analysis (rhythmic 500-2kHz)');
      }
    }
  }
   ...
   */
  private detectPowerTool(timestamp: number, bands: FrequencyBands): void {
    ...
    if (bands.lowMid > 150 && this.isSustained('lowMid', 140, 8)) {  // 800ms sustained
      ...
      if (!this.detectedEvents.has(eventKey)) {
        ...
        logger.debug(
          `[TW AudioFrequencyAnalyzer] 🔧 POWER TOOL DETECTED at ${timestamp.toFixed(1)}s`
        );

        // Power tools might indicate violence (chainsaw scenes)
        this.createWarning(eventKey, timestamp, 'violence', 60,
          'Power tool detected via frequency analysis (sustained 100-500Hz)');
      }
    }
  }
   ...
   */
  private isSustained(band: keyof FrequencyBands, threshold: number, checkCount: number): boolean {
   ...
   */
  private isFluctuating(band: keyof FrequencyBands, variance: number, checkCount: number): boolean {
    if (this.frequencyHistory.length < checkCount) {
      return false;
    }
   ...
   */
  private detectOscillation(band: keyof FrequencyBands, minCycles: number): boolean {
   ...
   */
  private detectPeriodicBeep(band: keyof FrequencyBands, threshold: number, checkCount: number): boolean {
    if (this.frequencyHistory.length < checkCount) {
      return false;
    }
