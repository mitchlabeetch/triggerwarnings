...
self.onmessage = (e) => {
    ...
    if (type === 'analyze_audio') {
        ...
        const frequencyData = new Uint8Array(p.frequencyData);
...
function analyzeAudio(frequencyData, timestamp, sampleRate, binCount) {
...
function calculateFrequencyBands(frequencyData, sampleRate, binCount) {
    return {
...
function getAverageFrequency(frequencyData, minHz, maxHz, sampleRate, binCount) {
...
// --- Detection Logic ---
function detectScream(timestamp, bands) {
    if (bands.highMid > 200 && bands.presence > 200 && bands.bass < 150) {
        if (isSustained('highMid', 180, 5)) {
            ...
            if (!detectedEvents.has(eventKey)) {
                ...
                createWarning(eventKey, timestamp, 'children_screaming', confidence, 'Screaming detected via frequency analysis (3-5kHz sustained)', bands);
            }
        }
    }
}
function detectGunshot(timestamp, bands) {
    if (frequencyHistory.length >= 2) {
        ...
        if (midIncrease > 100 && highMidIncrease > 100 && bands.mid > 180 && bands.highMid > 180) {
            ...
            if (!detectedEvents.has(eventKey)) {
                ...
                createWarning(eventKey, timestamp, 'violence', confidence, 'Gunshot detected via frequency analysis (1-5kHz transient)', bands);
            }
        }
    }
}
function detectExplosion(timestamp, bands) {
    if (bands.bass > 180 && bands.subBass > 150 && bands.mid > 120) {
        ...
        if (!detectedEvents.has(eventKey)) {
            ...
            createWarning(eventKey, timestamp, 'detonations_bombs', confidence, 'Explosion detected via frequency analysis (20-200Hz surge)', bands);
        }
    }
}
function detectSiren(timestamp, bands) {
    if (detectOscillation('mid', 2)) {
        const eventKey = `siren-${Math.floor(timestamp / 3) * 3}`;
        if (!detectedEvents.has(eventKey)) {
            stats.sirenDetections++;
            createWarning(eventKey, timestamp, 'violence', 75, 'Emergency siren detected via frequency analysis (oscillating 1-3kHz)', bands);
        }
    }
}
function detectMedicalBeep(timestamp, bands) {
    if (detectPeriodicBeep('mid', 140, 5)) {
        const eventKey = `medical-beep-${Math.floor(timestamp / 5) * 5}`;
        if (!detectedEvents.has(eventKey)) {
            stats.medicalBeepDetections++;
            createWarning(eventKey, timestamp, 'medical_procedures', 70, 'Medical equipment detected via frequency analysis (periodic 1-2kHz)', bands);
        }
    }
}
function detectCrying(timestamp, bands) {
    if (bands.mid > 120 && bands.mid < 180 && isFluctuating('mid', 30, 5)) {
        ...
        if (!detectedEvents.has(eventKey)) {
            ...
            createWarning(eventKey, timestamp, 'children_screaming', 65, 'Crying detected via frequency analysis (rhythmic 500-2kHz)', bands);
        }
    }
}
function detectPowerTool(timestamp, bands) {
    if (bands.lowMid > 150 && isSustained('lowMid', 140, 8)) {
        const eventKey = `power-tool-${Math.floor(timestamp / 5) * 5}`;
        if (!detectedEvents.has(eventKey)) {
            stats.powerToolDetections++;
            createWarning(eventKey, timestamp, 'violence', 60, 'Power tool detected via frequency analysis (sustained 100-500Hz)', bands);
        }
    }
}
// --- Helper Functions ---
function isSustained(band, threshold, checkCount) {
    if (frequencyHistory.length < checkCount)
        return false;
    const recent = frequencyHistory.slice(-checkCount);
    return recent.every(h => h[band] > threshold);
}
function isFluctuating(band, variance, checkCount) {
    if (frequencyHistory.length < checkCount)
        return false;
    const recent = frequencyHistory.slice(-checkCount);
...
function detectOscillation(band, minCycles) {
    if (frequencyHistory.length < 8)
        return false;
...
function detectPeriodicBeep(band, threshold, checkCount) {
    if (frequencyHistory.length < checkCount)
        return false;
    const recent = frequencyHistory.slice(-checkCount);
    const values = recent.map(h => h[band]);
...
function createWarning(eventKey, timestamp, category, confidence, description, frequencyProfile) {
    const event = {
        type: category,
        timestamp,
        confidence,
