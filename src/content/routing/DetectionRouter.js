...
export class DetectionRouter {
    ...
    route(category, input) {
        ...
        logger.debug(`[DetectionRouter] Routed ${category} via ${config.route} | ` +
     ...
     */
    calculateWeightedConfidence(input, weights) {
        let weightedSum = 0;
        let totalWeight = 0;
        if (input.visual) {
    ...
    static verifyCompleteness() {
        const allCategories = Object.keys(CATEGORY_ROUTE_CONFIG);
