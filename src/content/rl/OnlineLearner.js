...
export class OnlineLearner {
    ...
    BATCH_SIZE = 1; // True online (mini-batch = 1)
    MOMENTUM = 0.9; // SGD momentum
    L2_REGULARIZATION = 0.001; // Weight decay
    GRADIENT_CLIP = 5.0; // Gradient clipping
    // Drift detection hyperparameters
    DRIFT_WINDOW_SIZE = 100; // Window for drift detection
    DRIFT_THRESHOLD = 0.15; // Error increase threshold
    SUDDEN_DRIFT_THRESHOLD = 0.3; // Sudden drift threshold
    WARNING_THRESHOLD = 0.1; // Warning level
    // Drift tracking
    driftStats = new Map();
    ...
    update(example) {
        ...
        const gradient = this.computeGradient(example.features, prediction, target, weights, example.importance || 1.0);
        ...
        logger.debug(`[OnlineLearner] Updated ${example.category}: loss=${loss.toFixed(4)}, ` +
    ...
    batchUpdate(examples) {
        ...
        logger.info(`[OnlineLearner] 📦 Batch update: ${examples.length} examples, ` +
     ...
     */
    predictForCategory(features, category) {
     ...
     * Compute gradient
     */
    computeGradient(features, prediction, target, weights, importance) {
     ...
     */
    updateWeights(category, weights, gradient) {
    ...
    updateLearningRate() {
        ...
        // Time-based decay
        schedule.currentRate = Math.max(schedule.minRate, schedule.currentRate * schedule.decayFactor);
     ...
     */
    adjustLearningRate(performanceImproving) {
        const schedule = this.learningRateSchedule;
        ...
        logger.debug(`[OnlineLearner] Adjusted LR: improving=${performanceImproving}, ` +
     ...
     */
    trackForDrift(category, loss, prediction, target) {
     ...
     */
    updateDriftStats(driftStats) {
        if (driftStats.recentErrors.length === 0)
            return;
        // Compute mean
        const mean = driftStats.recentErrors.reduce((sum, e) => sum + e, 0) / driftStats.recentErrors.length;
        // Compute variance
        const variance = driftStats.recentErrors.reduce((sum, e) => sum + (e - mean) ** 2, 0) / driftStats.recentErrors.length;
    ...
    detectDrift(category) {
        ...
        if (driftDetected) {
            ...
            if (driftType === 'gradual')
                this.stats.gradualDrifts++;
            if (driftType === 'sudden')
                this.stats.suddenDrifts++;
            if (driftType === 'incremental')
                this.stats.incrementalDrifts++;
        }
        ...
        logger.debug(`[OnlineLearner] Drift detection: detected=${driftDetected}, ` +
     ...
     */
    computeDriftMagnitude(driftStats) {
        if (driftStats.recentErrors.length < 20)
            return 0;
