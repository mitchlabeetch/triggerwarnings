...
export type FeedbackType =
  | 'confirm'      // User confirmed detection was correct
  | 'dismiss'      // User dismissed warning (false positive)
  | 'report'       // User reported missed detection (false negative)
  | 'helpful'      // User marked as helpful
  | 'not-helpful'; // User marked as not helpful
...
export interface ShapedReward {
  immediateReward: number;      // Instant feedback reward
  delayedReward: number;        // Long-term satisfaction reward
  intrinsicReward: number;      // Exploration bonus
...
export class RewardShaper {
  // Reward parameters
  private readonly REWARD_CORRECT_DETECTION = 1.0;
  private readonly REWARD_FALSE_POSITIVE = -0.5;
  private readonly REWARD_FALSE_NEGATIVE = -0.8;
  private readonly REWARD_HELPFUL = 0.3;
  private readonly REWARD_NOT_HELPFUL = -0.2;

  // Reward shaping parameters
  private readonly CONFIDENCE_BONUS_SCALE = 0.2;
  private readonly TEMPORAL_DECAY = 0.95;
  private readonly EXPLORATION_BONUS_SCALE = 0.1;
  ...
  shapeReward(feedback: FeedbackEvent): ShapedReward {
    ...
    logger.debug(
      `[RewardShaper] ${feedback.type} → R=${totalReward.toFixed(2)} ` +
   ...
   */
  private computeImmediateReward(feedback: FeedbackEvent): number {
    let baseReward = 0;
   ...
   */
  private computeConfidenceBonus(feedback: FeedbackEvent): number {
   ...
   */
  private computeDelayedReward(feedback: FeedbackEvent): number {
   ...
   */
  private computeTemporalBonus(feedback: FeedbackEvent): number {
   ...
   */
  private computeIntrinsicReward(feedback: FeedbackEvent): number {
   ...
   */
  discountReward(reward: number, steps: number, gamma: number = 0.95): number {
   ...
   */
  private storeReward(category: TriggerCategory, reward: number): void {
  ...
  analyzeFeedback(category: TriggerCategory): {
    ...
    let rewardTrend: 'improving' | 'stable' | 'degrading';
   ...
   */
  private updateRewardVariance(reward: number): void {
