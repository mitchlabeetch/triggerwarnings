...
export interface TemporalWindow {
  ...
  sceneId?: string;  // Scene identifier if available
}
...
export interface RegularizedDetection extends Detection {
  originalConfidence: number;      // Before regularization
  regularizedConfidence: number;   // After regularization
  temporalBoost: number;           // Boost from adjacent detections
  temporalPenalty: number;         // Penalty from inconsistency
  coherenceScore: number;          // 0-100, how coherent with history
  shouldWarn: boolean;             // Final decision after regularization
}
...
export class TemporalCoherenceRegularizer {
  ...
  private detectionHistory: Map<TriggerCategory, Detection[]> = new Map();

  // Scene context history
  private sceneHistory: SceneContext[] = [];

  // Configuration
  private readonly HISTORY_WINDOW = 30;  // Keep 30 seconds of history
  private readonly ADJACENT_WINDOW = 3;  // Consider detections within 3 seconds as adjacent
  private readonly SMOOTHING_FACTOR = 0.3;  // Weight for temporal smoothing
  private readonly MIN_SUSTAINED_DURATION = 2;  // Minimum 2 seconds for sustained detection
  private readonly JUMP_THRESHOLD = 30;  // Confidence jump >30% is suspicious
  ...
  regularize(detection: Detection, currentTime: number): RegularizedDetection {
    ...
    const shouldWarn = this.shouldWarnDecision(
      detection,
    ...
    const regularized: RegularizedDetection = {
      ...detection,
    ...
    logger.debug(
      `[TemporalCoherence] ${detection.category} | ` +
   ...
   */
  private calculateCoherence(detection: Detection, history: Detection[]): number {
    if (history.length === 0) {
      return 50;  // Neutral coherence for first detection
    }

    // Find adjacent detections (within ADJACENT_WINDOW)
    const adjacent = history.filter(d =>
      Math.abs(d.timestamp - detection.timestamp) <= this.ADJACENT_WINDOW
    );
   ...
   */
  private calculateTemporalBoost(detection: Detection, history: Detection[]): number {
    const adjacent = history.filter(d =>
      Math.abs(d.timestamp - detection.timestamp) <= this.ADJACENT_WINDOW &&
   ...
   */
  private calculateTemporalPenalty(detection: Detection, history: Detection[]): number {
    if (history.length === 0) {
   ...
   */
  private applySmoothhing(detection: Detection, history: Detection[]): number {
    if (history.length === 0) {
    ...
    const recent = history.filter(d =>
      detection.timestamp - d.timestamp <= 5
    );
   ...
   */
  private applySceneContext(detection: Detection, currentTime: number): number {
    // Find current scene
    const currentScene = this.sceneHistory.find(scene =>
    ...
    const adjustments: Partial<Record<TriggerCategory, Record<string, number>>> = {
      'medical_procedures': {
   ...
   */
  private isSustained(detection: Detection, history: Detection[]): boolean {
    // Find detections in sustained window
    const sustained = history.filter(d =>
      d.timestamp >= detection.timestamp - this.MIN_SUSTAINED_DURATION &&
  ...
  private cleanHistory(currentTime: number): void {
    ...
    for (const [category, history] of this.detectionHistory.entries()) {
      this.detectionHistory.set(
        category,
        history.filter(d => d.timestamp >= cutoff)
      );
    }
